 ...
 */
public class VScrollTextView extends RelativeLayout {
    /**
     * 默认轮播时间间隔：3s
     */
    ...
    public VScrollTextView(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.VScrollTextView);
        sleepTime = array.getInt(R.styleable.VScrollTextView_sleepTime, SLEEP_TIME);
        animDuration = array.getInt(R.styleable.VScrollTextView_animDuration, ANIM_DURATION);
        scrollOrientation = array.getInt(R.styleable.VScrollTextView_scrollOrientation, 0);
        mTextSize = array.getDimensionPixelSize(R.styleable.VScrollTextView_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
                12, getResources().getDisplayMetrics()));
        mTextColor = array.getColor(R.styleable.VScrollTextView_textColor, Color.BLACK);
        mSingleLine = array.getBoolean(R.styleable.VScrollTextView_singleLine, true);
        array.recycle();
    ...
    private void autoSlider() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            //Andorid 3.0以下不处理动画效果
            mTvContentTop.setText(next);
            mTvContentBottom.setVisibility(GONE);
        }
    }
    ...
    private static class SliderScrollHandler extends Handler {
        private WeakReference<VScrollTextView> mSliderView;

        SliderScrollHandler(VScrollTextView sliderView) {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    VScrollTextView sliderView = mSliderView.get();
